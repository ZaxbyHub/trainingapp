...
try:
    ...
    from customtkinter import CTkProgressBar, CTkOptionMenu, CTkScrollableFrame, CTkToplevel, CTkSwitch
    GUI_AVAILABLE = True
except ImportError:
    GUI_AVAILABLE = False
...
class SettingsDialog(CTkToplevel):
    ...
    def _create_widgets(self):
        ...
        # LLM Settings
        CTkLabel(main_frame, text="LLM Settings", font=("", 16, "bold")).pack(pady=(0, 10))
        ...
        CTkButton(model_frame, text="Browse", width=70, command=self._browse_model).pack(side="left")
        ...
        # RAG Settings
        CTkLabel(main_frame, text="RAG Settings", font=("", 16, "bold")).pack(pady=(20, 10))
        
        settings_frame = CTkFrame(main_frame)
        settings_frame.pack(fill="x")
        
        CTkLabel(settings_frame, text="Chunk Size:").grid(row=0, column=0, sticky="w", pady=5)
        self.chunk_size_entry = CTkEntry(settings_frame, width=100)
        self.chunk_size_entry.grid(row=0, column=1, padx=10, pady=5)
        
        CTkLabel(settings_frame, text="Results to Retrieve:").grid(row=1, column=0, sticky="w", pady=5)
        self.n_results_entry = CTkEntry(settings_frame, width=100)
        self.n_results_entry.grid(row=1, column=1, padx=10, pady=5)
        
        CTkLabel(settings_frame, text="Max Tokens:").grid(row=2, column=0, sticky="w", pady=5)
        self.max_tokens_entry = CTkEntry(settings_frame, width=100)
        self.max_tokens_entry.grid(row=2, column=1, padx=10, pady=5)
        
        CTkLabel(settings_frame, text="Temperature:").grid(row=3, column=0, sticky="w", pady=5)
        self.temperature_entry = CTkEntry(settings_frame, width=100)
        self.temperature_entry.grid(row=3, column=1, padx=10, pady=5)
        
        # Advanced RAG Settings
        CTkLabel(main_frame, text="Advanced RAG Settings", font=("", 16, "bold")).pack(pady=(20, 10))

        advanced_frame = CTkFrame(main_frame)
        advanced_frame.pack(fill="x")

        # Hybrid Search toggle
        CTkLabel(advanced_frame, text="Hybrid Search (BM25 + Vector):").grid(row=0, column=0, sticky="w", pady=5)
        self.hybrid_search_var = tk.StringVar(value="on" if self.settings.get("hybrid_search", True) else "off")
        self.hybrid_switch = CTkSwitch(advanced_frame, text="", variable=self.hybrid_search_var, onvalue="on", offvalue="off")
        self.hybrid_switch.grid(row=0, column=1, padx=10, pady=5)

        # Window Expansion
        CTkLabel(advanced_frame, text="Window Expansion (chunks):").grid(row=1, column=0, sticky="w", pady=5)
        ...
        # Reranking toggle
        CTkLabel(advanced_frame, text="Cross-Encoder Reranking:").grid(row=2, column=0, sticky="w", pady=5)
        self.reranking_var = tk.StringVar(value="on" if self.settings.get("reranking_enabled", False) else "off")
        self.reranking_switch = CTkSwitch(advanced_frame, text="", variable=self.reranking_var, onvalue="on", offvalue="off")
        ...
        CTkButton(button_frame, text="Cancel", command=self.destroy).pack(side="right", padx=5)
        CTkButton(button_frame, text="Save", command=self._save).pack(side="right", padx=5)
    
    def _browse_model(self):
        path = filedialog.askopenfilename(title="Select GGUF Model File", filetypes=[("GGUF files", "*.gguf")])
        if path:
    ...
    def _populate_fields(self):
        ...
        self.ollama_model_entry.insert(0, self.settings.get("ollama_model", "phi3:mini"))
        ...
        self.hybrid_search_var.set("on" if self.settings.get("hybrid_search", True) else "off")
        self.retrieval_window_entry.insert(0, str(self.settings.get("retrieval_window", 1)))
        self.reranking_var.set("on" if self.settings.get("reranking_enabled", False) else "off")
...
class DocumentQAApp(CTk):
    ...
    def _create_widgets(self):
        ...
        CTkButton(top_bar, text="⚙ Settings", width=100, command=self._open_settings).pack(side="right", padx=5)
        CTkButton(top_bar, text="📁 Ingest", width=100, command=self._ingest_documents).pack(side="right", padx=5)
        
        # Status bar
        self.status_frame = CTkFrame(self)
        self.status_frame.pack(fill="x", padx=10, pady=5)
        
        self.status_label = CTkLabel(self.status_frame, text="Initializing...", font=("", 12))
        ...
        self.doc_count_label = CTkLabel(self.status_frame, text="Documents: 0", font=("", 12))
        ...
        # Welcome message
        self._add_message("system", f"Welcome to {self.APP_NAME}!\n\n"
        ...
        self.question_entry = CTkEntry(input_frame, placeholder_text="Ask a question about your documents...")
        ...
        self.ask_button = CTkButton(input_frame, text="Ask", width=80, command=self._ask_question)
        self.ask_button.pack(side="right")
        
        # Clear button
        CTkButton(input_frame, text="Clear", width=60, command=self._clear_chat).pack(side="right", padx=5)
    ...
    def _initialize_engine(self):
        ...
        def init():
            try:
                ...
                self.message_queue.put(("message", "system", f"Failed to initialize: {e}\n\nPlease check Settings."))
    ...
    def _open_settings(self):
        ...
        if dialog.result:
            ...
            if messagebox.askyesno("Restart Required", 
                                   "Settings changed. Restart the engine now?"):
    ...
    def _ingest_documents(self):
        ...
        def ingest():
            try:
                ...
                if stats["success"]:
                    self.message_queue.put(("message", "system", 
                        f"✓ Ingested {stats['documents']} documents "
                        f"({stats['chunks_added']} new chunks) "
                        f"in {stats['time_seconds']:.1f}s"))
                    self.message_queue.put(("doc_count", 
                        self.engine.get_stats()["document_count"]))
                else:
                    self.message_queue.put(("message", "system", 
