...
class TestIngestDirectoryStats:
    ...
    def test_ingest_directory_success(self, tmp_path):
        ...
        # Create a text file
        (test_dir / "test1.txt").write_text("This is test document one with some content.")
        (test_dir / "test2.txt").write_text("This is test document two with different content.")
            ...
            with patch('rag_engine.DocumentProcessor') as mock_processor:
                with patch('rag_engine.SmartLLM'):
                    with patch('rag_engine.RAGEngine._save_config'):
                        ...
                        mock_store_instance.add_chunks.return_value = 4  # 2 chunks per file
                        mock_vector_store.return_value = mock_store_instance
                        
                        mock_processor_instance = MagicMock()
                        mock_processor_instance.process_directory.return_value = [
                            DocumentChunk(text="chunk1", source="test1.txt", chunk_index=0),
                            DocumentChunk(text="chunk2", source="test1.txt", chunk_index=1),
                            DocumentChunk(text="chunk3", source="test2.txt", chunk_index=0),
                            DocumentChunk(text="chunk4", source="test2.txt", chunk_index=1),
                        ]
            ...
            with patch('rag_engine.DocumentProcessor') as mock_processor:
                with patch('rag_engine.SmartLLM'):
                    with patch('rag_engine.RAGEngine._save_config'):
                        ...
                        mock_processor_instance = MagicMock()
                        mock_processor_instance.process_directory.return_value = [
                            DocumentChunk(text="chunk", source="test.txt", chunk_index=0)
                        ]
                        mock_processor.return_value = mock_processor_instance
                        
                        engine = RAGEngine()
                        stats = engine.ingest_directory(str(test_dir), callback=callback)
        ...
        with patch('rag_engine.VectorStore') as mock_vector_store:
            with patch('rag_engine.SmartLLM') as mock_llm:
                with patch('rag_engine.RAGEngine._save_config'):
                    ...
                    mock_llm_instance.answer_question.return_value = "This is the answer based on context."
                    ...
                    assert "answer" in result.answer.lower() or "This is the answer" in result.answer
        ...
        with patch('rag_engine.VectorStore') as mock_vector_store:
            with patch('rag_engine.SmartLLM') as mock_llm:
                with patch('rag_engine.RAGEngine._save_config'):
                    ...
                    mock_llm_instance = MagicMock()
                    mock_llm_instance.answer_question.return_value = "Hello! How can I help you?"
                    mock_llm.return_value = mock_llm_instance
        ...
        with patch('rag_engine.VectorStore') as mock_vector_store:
            with patch('rag_engine.SmartLLM') as mock_llm:
                with patch('rag_engine.RAGEngine._save_config'):
                    ...
                    mock_llm_instance.answer_question.return_value = "I couldn't find any relevant information in the documents."
                    mock_llm.return_value = mock_llm_instance
...
class TestRAGConfig:
    ...
    def test_config_backward_compatibility(self):
        ...
        assert config.db_path == "./test_db"
        assert config.chunk_size == 256  # Default
