...
def test_validate_url_accepts_nonstandard_ports_when_allowed():
    ...
    nonstandard_local_urls = [
        (
            ...
            {80, 443, 11434},
        ),  # Ollama default port (in DEFAULT_ALLOWED_PORTS)
...
def test_lifespan_allows_localhost_for_ollama():
    """
    Test that validate_url accepts localhost URLs for Ollama connections
    when allow_local=True is passed, as the lifespan function should do.
