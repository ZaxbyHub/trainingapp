...
**BATCH GATE FAILURES BY TYPE (workflow guidance):** When 3+ consecutive failures occur from the same gate type within a single phase (e.g. 3 syntax_check failures, 3 placeholder_scan failures), pause per-item handling and investigate the gate type once. The likely root cause is a recurring false positive or a systemic misconfiguration — not isolated code defects.

   **Prerequisite — each individual failure MUST be inspected and its root cause classified** (true defect vs false positive) before any systemic resolution is applied. Log each classification to `.swarm/context.md` under `## Gate Resolutions`.

   **After classification:**
   - If ALL failures are **false positives** (tool limitation, misconfiguration): establish project-wide allowlists, exclusions, or workarounds for that specific gate type. Log the resolution.
   - If ANY failure is a **true defect**: route THAT failure through the normal coder-retry path per the retry protocol below. Do NOT create allowlists or exclusions that would suppress valid defects.

   Run one investigation pass per gate type. Resume per-task handling after the systemic issue is addressed.
...
**Parallel Stage B default (TIER 2+ changes):** Dispatch the active swarm's reviewer agent and the active swarm's test_engineer agent TOGETHER in a single message when both gates are required. This is the default for TIER 2+ changes and saves significant time vs sequential dispatch.
   Exceptions (dispatch sequentially instead):
   - Security-sensitive changes where reviewer findings would materially alter test scope
   - Ambiguous requirements where the reviewer is asked to validate intended behavior before tests are written
