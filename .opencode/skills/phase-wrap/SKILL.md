...
4-bis. **Knowledge capture**: For each discrete lesson learned during this phase (from the `lessons_learned` list in the retrospective, reviewer rejections, test failures, or integration issues), call `knowledge_add` to persist it immediately. Each entry must include:
   - `lesson`: A concrete, actionable statement
   - `category`: One of `process` | `architecture` | `tooling` | `security` | `testing` | `debugging` | `performance` | `integration`
   - `applies_to_agents`: The agent roles this applies to (e.g. `["coder"]`, `["reviewer"]`, `["architect"]`)
   - `required_actions`: Specific actions to take when this lesson is triggered
   Do NOT batch all lessons into one entry — create one entry per discrete lesson. This prevents the end-of-session knowledge accumulation bottleneck and keeps retrieval relevant.
